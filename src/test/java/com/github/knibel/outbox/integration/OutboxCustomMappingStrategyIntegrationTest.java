...
@SpringBootTest(
        ...
        properties = {
                ...
                "outbox.tables[0].fieldMappings.order_id=orderId",
                "outbox.tables[0].fieldMappings.customer_name=customer.name",
                "outbox.tables[0].fieldMappings.customer_email=customer.email",
                "outbox.tables[0].fieldMappings.city=customer.address.city",
...
class OutboxCustomMappingStrategyIntegrationTest {
    ...
    void shouldPublishNestedJsonPayloadToKafka() throws Exception {
        ...
        jdbcTemplate.update(
                "INSERT INTO custom_mapping_outbox (id, order_id, customer_name, customer_email, city, status) "
                + "VALUES (?, ?, ?, ?, ?, 'PENDING')",
                id, "ORD-001", "John Doe", "john@example.com", "Berlin");
    ...
    void shouldProcessMultipleRowsWithCustomMapping() {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            jdbcTemplate.update(
                    "INSERT INTO custom_mapping_outbox (id, order_id, customer_name, customer_email, city, status) "
                    + "VALUES (?, ?, ?, ?, ?, 'PENDING')",
                    id, "ORD-" + i, "Customer " + i, "cust" + i + "@test.com", "City " + i);
        }
