 ...
 *   <li><b>Post-publish</b> – apply the configured {@link PostPublishStrategy}
 *       (e.g. mark rows as done or delete them).
 * </ol>
 *
...
public class OutboxPoller {
    ...
    private final PostPublishStrategy postPublishStrategy;
    ...
    OutboxPoller(OutboxTableProperties config,
                 ...
                 PostPublishStrategy postPublishStrategy,
                 MeterRegistry meterRegistry) {
        this.config = config;
        this.repository = repository;
        this.kafkaProducer = kafkaProducer;
        this.postPublishStrategy = postPublishStrategy;
    ...
    public void poll() {
        ...
        postPublishStrategy.onSuccess(config, ids, repository);
        doneCounter.increment(ids.size());
        log.debug("Applied post-publish strategy '{}' for {} row(s) in table '{}'",
                config.getProcessingStrategy(), ids.size(), config.getTableName());
    }
